...
export class SessionParser {
  ...
  async analyze(filePath: string) {
    ...
    const geminiMessages = session.messages.filter(
      (m: { type: string }) => m.type === 'gemini',
    );
    ...
    // 3. 提取工具链 (用于模式识别)
    const toolChain = geminiMessages.flatMap(
      (m: { toolCalls?: { name: string }[] }) =>
        (m.toolCalls || []).map((tc) => tc.name),
    );

    // 4. 识别纠错行为 (检测用户的不满或修正习惯)
    const correctionCount = userMessages.filter(
  ...
  private generateSuggestion(prompt: string): string {
    // 简易逻辑：如果指令太短，建议增加上下文
    if (prompt.length < 10)
      return '指令过于简短，建议补充具体的文件背景或预期目标。';
