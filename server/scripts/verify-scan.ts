...
async function main() {
  ...
  const checkpoints = results.filter(r => path.basename(r.filePath).startsWith('checkpoint-'));
  ...
  if (checkpoints.length > 0) {
    ...
    checkpoints.slice(0, 5).forEach(c => console.log(`  - [${c.projectName}] ${path.basename(c.filePath)}`));
  } else {
