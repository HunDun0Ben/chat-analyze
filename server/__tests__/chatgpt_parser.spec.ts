...
describe('SessionParser (ChatGPT Unification)', () => {
  ...
  it('should unify fragmented ChatGPT nodes into a single structured message', async () => {
    const tempDir = path.join(
      os.tmpdir(),
      'chat-analyze-chatgpt-test-' + Date.now(),
    );
      ...
      mapping: {
        ...
        'node-3': {
          ...
          message: {
            ...
            content: {
              content_type: 'code',
              text: 'search("ApsaraMQ for Kafka")',
            },
        ...
        'node-4': {
          ...
          message: {
            ...
            author: { role: 'tool' },
            content: {
              text: 'ApsaraMQ for Kafka is a fully managed message queue service.',
            },
        ...
        'node-5': {
          ...
          message: {
            ...
            content: {
              parts: [
                'Here is what I found: ApsaraMQ for Kafka is a managed service.',
              ],
            },
    ...
    expect(assistantMsg.thoughts?.[0].description).toBe(
      'I need to search for ApsaraMQ for Kafka.',
    );

    // Check tool calls
    expect(assistantMsg.toolCalls?.length).toBe(1);
    expect(assistantMsg.toolCalls?.[0].name).toBe('search');
    expect(assistantMsg.toolCalls?.[0].args.code).toBe(
      'search("ApsaraMQ for Kafka")',
    );
    expect(assistantMsg.toolCalls?.[0].result).toContain(
  ...
  it('should handle multiple user turns correctly', async () => {
    const tempDir = path.join(
      os.tmpdir(),
      'chat-analyze-chatgpt-multi-' + Date.now(),
    );
    await fs.mkdir(tempDir, { recursive: true });
